        ...
        >
          ...
          <header className="h-[60px] shrink-0 border-b border-[#EEF2F8] px-5 flex items-center justify-between bg-white/95 backdrop-blur-md sticky top-0 z-20">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              {category && (
                <>
                  {/* Mobile & Tablet: Icon only circular badge */}
                  <div
                    className={`flex xl:hidden w-7 h-7 rounded-full items-center justify-center ${category.badgeColor} text-white shrink-0 shadow-sm border border-white/10`}
                  >
                    <category.icon size={14} strokeWidth={2.5} />
                  </div>
                  {/* Desktop: Full text badge */}
                  <div
                    className={`hidden xl:block px-2.5 py-0.5 rounded-md ${category.badgeColor} text-white text-[9px] font-black tracking-widest uppercase shrink-0 shadow-sm`}
                  >
                    {category.name}
                  </div>
                </>
              ...
              >
                {item.type === "video" && (
                  <div className="flex-1 flex flex-col bg-black relative">
                    ...
                    {/* Floating Overlay Footer (Avoiding YouTube Controls) */}
                    <div className="absolute bottom-[clamp(48px,8vh,80px)] left-6 z-40">
                      <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        className="bg-white/90 backdrop-blur-md px-4 py-2.5 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.15)] flex items-center gap-5 border border-white/40"
                      >
                        <motion.button
                          onClick={(e) => handleToggleBookmark(item.id, e)}
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className="flex items-center gap-2 font-bold text-[11px] text-[#223654] transition-colors"
                        >
                          <motion.div
                            animate={
                              item.bookmarked
                          ...
                          >
                            <Bookmark
                              size={14}
                              className={`transition-all duration-300 ${item.bookmarked ? "fill-[#F6C445] text-[#F6C445]" : "text-[#223654]"}`}
                            />
                          </motion.div>
                          <span>
                        ...
                        <div className="flex items-center gap-2 text-[#223654] font-bold text-[11px]">
                          <Eye size={14} />
