...
export default function LoginScreen() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFF2E6] via-[#FDF5E6] to-[#E6F9F0] relative flex flex-col items-center md:justify-center p-3 sm:p-6 md:p-8 xl:p-20 font-sans overflow-x-hidden">
      ...
      {/* Main Container for Tablet & Desktop 2-Column Layout */}
      <div className="w-full max-w-[1280px] mx-auto flex flex-col md:flex-row md:items-center md:gap-8 xl:gap-12 2xl:gap-20 relative z-10">
        
        {/* Hero Illustration Area */}
        <div className="w-full md:w-[45%] xl:w-1/2 flex items-center justify-center mt-2 sm:mt-8 md:mt-0 order-1 md:order-none md:relative md:bg-gradient-to-b md:from-[#FFF5EE] md:to-[#FFF9E5] md:min-h-[400px] xl:min-h-[600px] md:rounded-[48px] xl:rounded-[64px]">
          {/* Decorative Doodles for Tablet & Desktop Illustration */}
          <div className="hidden md:block absolute inset-0 pointer-events-none overflow-hidden">
            <motion.div 
              animate={{ y: [0, -15, 0], rotate: [0, 10, 0] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
              className="absolute top-8 left-8 xl:top-12 xl:left-12 text-[#FF7A90]/20"
            >
            ...
            <motion.div 
              ...
              className="absolute bottom-16 left-12 xl:bottom-20 xl:left-16 text-[#FFD166]/30"
            >
            ...
            <motion.div 
              ...
              className="absolute top-16 right-12 xl:top-20 xl:right-16 text-[#86EFAC]/30"
            >
            ...
            <motion.div 
              ...
              className="absolute bottom-12 right-12 xl:bottom-16 xl:right-20 text-[#A5B4FC]/30"
            >
              <Flower2 size={28} fill="currentColor" />
            </motion.div>
            <motion.div 
              animate={{ opacity: [0.3, 0.6, 0.3], scale: [1, 1.2, 1] }}
              transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
              className="absolute top-1/2 left-6 text-[#FFB6C1]/40"
            >
              <Sparkles size={20} />
            </motion.div>
          </div>
          ...
          <motion.div 
            ...
            className="relative w-full flex items-center justify-center h-[140px] sm:h-[240px] md:h-[350px] xl:h-[75vh]"
          >
            {/* Main Illustration */}
            <div className="w-full h-full flex justify-center items-center relative z-20">
              <img 
                ...
                className="max-h-[90%] w-auto object-contain z-20 drop-shadow-xl"
              />
            </div>
          </motion.div>
        </div>
        ...
        {/* Login Card */}
        <div className="w-full md:w-[55%] xl:w-1/2 flex justify-center md:justify-center xl:justify-start order-2 md:order-none mt-8 sm:mt-12 md:mt-0">
          <motion.div 
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}
            className="w-full max-w-[460px] md:max-w-[420px] xl:max-w-[480px] bg-white rounded-[32px] sm:rounded-[40px] md:rounded-[48px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-6 sm:p-8 md:p-10 xl:p-16 relative z-20"
          >
            {/* School Logo */}
            <div className="flex justify-center mb-4 sm:mb-6 -mt-12 sm:-mt-16 md:-mt-20 relative z-30">
              <div className="w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 bg-white rounded-full p-2 md:p-3 shadow-[0_8px_24px_rgba(0,0,0,0.06)] border border-gray-50 flex items-center justify-center">
            ...
            <div className="text-center mb-6 sm:mb-10">
              <h2 className="text-[#FF7A90] font-bold tracking-[0.2em] text-[12px] md:text-[14px] uppercase mb-2 sm:mb-3">
                Kho Học Liệu Số
              </h2>
              <h1 className="text-2xl sm:text-3xl md:text-4xl xl:text-[42px] font-black leading-[1.1] mb-3 sm:mb-6 tracking-tight py-1">
                <span className="bg-gradient-to-r from-[#FF7A90] via-[#FFB6C1] to-[#FFD166] text-transparent bg-clip-text drop-shadow-sm filter">
                  Trường Mầm Non<br className="hidden md:block" /> Hoa Hồng Nhỏ
                </span>
              </h1>
              
              {/* Decorative Divider */}
              <div className="flex items-center justify-center gap-2 mb-4 sm:mb-6">
                <div className="w-12 h-1.5 md:h-2 bg-[#FFD166] rounded-full"></div>
                <div className="w-1.5 h-1.5 md:h-2 md:w-2 bg-[#FFD166] rounded-full"></div>
              </div>

              <p className="text-[#6B7280] text-[14px] md:text-[17px] leading-relaxed px-1 md:px-4 font-medium">
            ...
            <form onSubmit={handleLogin} className="space-y-4 sm:space-y-6">
              <div className="space-y-2">
                <label htmlFor="parentName" className="block text-[14px] md:text-[16px] font-bold text-[#1C325B] ml-1">
                  Tên phụ huynh
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <User className={`h-[20px] w-[20px] md:h-[24px] md:w-[24px] transition-colors ${error ? 'text-red-400' : 'text-[#9CA3AF] group-focus-within:text-[#007AFF]'}`} />
                  </div>
                  <input
                    id="parentName"
                    type="text"
                    ...
                    className={`w-full h-[50px] md:h-[60px] pl-12 md:pl-14 pr-4 bg-white border-2 rounded-[20px] md:rounded-[24px] outline-none transition-all text-[15px] md:text-[17px] text-[#1C325B] placeholder:text-[#9CA3AF] font-medium ${
                      error 
                ...
                {error && (
                  <motion.p 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="text-red-500 text-[13px] md:text-[14px] mt-2 ml-2 font-bold flex items-center gap-1.5"
                  >
              ...
              <button
                type="submit"
                className="w-full h-[50px] md:h-[60px] mt-1 sm:mt-4 bg-[#007AFF] hover:bg-[#0066CC] active:bg-[#0052A3] text-white font-bold text-[16px] md:text-[18px] rounded-[20px] md:rounded-[24px] shadow-[0_6px_20px_rgba(0,122,255,0.25)] hover:-translate-y-[2px] active:translate-y-0 active:scale-[0.98] transition-all flex justify-center items-center gap-2.5 group"
              >
                <BookOpen size={18} strokeWidth={2.5} className="opacity-90" />
                <span>Vào xem học liệu</span>
                <ArrowRight size={18} strokeWidth={2.5} className="transition-transform group-hover:translate-x-1 opacity-90" />
              </button>
            </form>
